...
func setup(c bn.NodeClient, h data.HTTPClient) {
	...
	err = h.Do(context.Background(), http.MethodPost, "http://localhost:8446/api/p2pDest/epic@nchain.com", nil, http.StatusOK, &req, &res)
	if err != nil {
		panic(err)
	}
	...
	err = h.Do(context.Background(), http.MethodPost, "http://localhost:8446/api/p2pRawTx", nil, http.StatusOK, &reqraw, &resraw)
	if err != nil {
		panic(err)
	}
