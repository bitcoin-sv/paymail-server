...
func setup(c bn.NodeClient, h data.HTTPClient) {
	...
	err = h.Do(context.Background(), http.MethodPost, "http://localhost:8446/api/p2pDest/epic@nchain.com", http.StatusOK, nil, &req, &res)
	if err != nil {
		panic(err)
	}
	...
	err = h.Do(context.Background(), http.MethodPost, "http://localhost:8446/api/p2pRawTx", http.StatusOK, nil, &reqraw, &resraw)
	if err != nil {
		panic(err)
	}
